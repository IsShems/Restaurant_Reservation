              ...
              <React.Fragment key={`zone-block-${zone.key}`}>
                ...
                {zoneTables.map((table) => {
                  ...
                  if (usePersistedLayout) {
                    const persistedX = Number(table.positionX);
                    const persistedY = Number(table.positionY);
                    if (
                      Number.isFinite(persistedX) &&
                      Number.isFinite(persistedY)
                    ) {
                      const isRelativeEncoded =
                        persistedX > RELATIVE_COORD_THRESHOLD ||
                        persistedY > RELATIVE_COORD_THRESHOLD;

                      if (isRelativeEncoded) {
                        renderX =
                          (persistedX / RELATIVE_COORD_SCALE) * canvasSize.w;
                        finalY =
                          (persistedY / RELATIVE_COORD_SCALE) * canvasSize.h;
                      } else {
                        renderX = persistedX;
                        finalY = persistedY;
                      }
